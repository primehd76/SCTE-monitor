...
def probe_stream(url):
    ...
    try:
        ...
        # Cari stream video berdasarkan codec_type, bukan berdasarkan urutan index
        ...
        if not video_info: 
            return {"status": "error", "msg": "Tidak ditemukan stream video yang valid"}

        # Kalkulasi Format (1080i25 dari field_order: tt/bb)
    ...
    try:
        while True:
            ...
            if data["action"] == "play":
                ...
                # Perintah FFmpeg universal yang aman untuk video tanpa audio
                ffmpeg_cmd = [
                    "ffmpeg", "-hide_banner", "-loglevel", "error",
                    "-i", url,
                    # Routing video ke MediaMTX (biarkan audio opsional jika ada)
                    "-map", "0:v?", "-map", "0:a?", "-c", "copy", "-f", "rtsp", "rtsp://mediamtx:8554/live",
                    # Copy seluruh stream untuk parser SCTE-35
